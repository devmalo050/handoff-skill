...
### Claude Code (또는 filesystem MCP 연결된 데스크탑)

`skills/` 아래 두 디렉토리를 `~/.claude/skills/` 로 복사하면 전역에서 잡힙니다(특정 프로젝트에서만 쓰려면 그 레포의 `.claude/skills/` 로):

```bash
git clone https://github.com/devmalo050/handoff-skill.git
mkdir -p ~/.claude/skills
cp -R handoff-skill/skills/handoff ~/.claude/skills/
cp -R handoff-skill/skills/handoff-continue ~/.claude/skills/
```

새 세션에서 `/handoff`, `/handoff-continue` 로 호출되고, "핸드오프 해줘" 같은 자연어로도 트리거됩니다.

### Claude 데스크탑 앱 / claude.ai 웹

업로드용 zip을 만들어 설정에서 올립니다:

```bash
git clone https://github.com/devmalo050/handoff-skill.git
cd handoff-skill
bash scripts/build-zips.sh        # dist/handoff.zip, dist/handoff-continue.zip 생성
```

그 다음 **Settings → Customize → Skills → Create skill → Upload a skill** 에서 `dist/handoff.zip` 과 `dist/handoff-continue.zip` 을 각각 업로드하고 토글을 켭니다.

> 데스크탑 앱/웹의 Skill은 격리된 샌드박스에서 실행돼 로컬 파일·git에 직접 접근하지 못합니다. 이때 handoff는 **휴대 모드**로 동작합니다(아래 "환경별 동작" 참고).
...
│   └── handoff-continue/SKILL.md  # handoff-continue
└── scripts/
    └── build-zips.sh              # claude.ai 업로드용 zip 생성
```
...
Skill은 평범한 마크다운 + YAML frontmatter입니다. `skills/*/SKILL.md` 를 열어 프롬프트를 직접 고치면 됩니다. 예:
- 템플릿 섹션 추가/삭제
- 출력 언어 고정 (기본은 "세션에서 쓰던 언어")
- 커밋 동작 끄기 / 커밋 메시지 형식 변경

수정 후에는 `~/.claude/skills/` 의 파일을 다시 덮어쓰거나(Claude Code), `scripts/build-zips.sh` 로 zip을 다시 만들어 재업로드(데스크탑 앱/웹)하면 적용됩니다.
